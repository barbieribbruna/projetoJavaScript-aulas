...
const setFrutas: Set<string> = new Set<string>(["Abacate", "Banana", "Jaboticaba", "Kiwi", "Maçã", "Morango", "Pêra", "Jaboticaba"])
                            // não permite a palavra duplicada (ignora-se se há duplicação)
...
for(let fruta of setFrutas) // percorre todos os elementos do Set, um por um, e exibe cada fruta armazenada
{
    console.log(fruta)
}

let valor: string = leia.question("Insira uma fruta: ")
...
console.log("\nExiste a fruta Kiwi? " + setFrutas.has("Kiwi")) // ,has verifica se um valor específico existe dentro do Set e retorna um valor booleano
